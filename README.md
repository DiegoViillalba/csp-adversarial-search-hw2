...
`results/solutions/*` (la solución concreta encontrada).
...
- **En el remoto**, tras clonar este repo e instalar dependencias, `scripts/run_remote_heavy.sh`
  corre los cuatro comandos de arriba en background con `nohup` y deja logs en `logs/`. Sus
  resultados se traen de vuelta con `rsync` a `results/` y `data/graphs/`, y se usan igual que si
  se hubieran generado localmente — ver comentarios dentro del script para el uso exacto.
